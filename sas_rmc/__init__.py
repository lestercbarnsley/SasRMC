#%%
from sas_rmc.vector import Vector
from sas_rmc.particles.particle import Particle
from sas_rmc.box_simulation import Box
from sas_rmc import shapes
'''
from sas_rmc.shapes import shapes
from sas_rmc.particles import particle
from sas_rmc import commands, scattering_simulation, simulator, acceptance_scheme, viewer, array_cache, result_calculator, form_calculator

from sas_rmc.factories.runner_factory import load_config
from sas_rmc.factories.simulation_factory import box_simulation_params_factory
from sas_rmc.template_generator import generate_template

# Only very import classes should be directly imported
# This is just for putting things into the namespace
from sas_rmc.particles.particle import Particle
from sas_rmc.vector import Vector, VectorSpace, VectorElement
from sas_rmc.detector import DetectorImage, SimulatedDetectorImage, DetectorPixel, DetectorConfig, Polarization
from sas_rmc.controller import Controller
from sas_rmc.logger import Logger
'''

if __name__ == "__main__":
    pass
    
# %%