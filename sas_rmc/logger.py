...
from .command_writer import CommandWriter, Loggable
...
class Logger:
    ...
    _particle_log: List[pd.DataFrame] = field(default_factory=list, init=False, repr=False)
    ...
    def __enter__(self):
        self._particle_log = box_list_to_panda_list(self.box_list)
        return self

    def write_output(self) -> None:
        with pd.ExcelWriter(self.save_file_path) as writer:
            for i, initial_particle_log in enumerate(self._particle_log):
            ...
            for i, final_particle_log in enumerate(box_list_to_panda_list(self.box_list)):
